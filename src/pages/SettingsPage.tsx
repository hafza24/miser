    ...
    <AppLayout>
      <div className="p-4 space-y-6 animate-fade-in pb-12">
        ...
        <section className="bg-card rounded-2xl p-6 shadow-card space-y-3">
          ...
          <p className="text-xs text-muted-foreground">Pick what you're here for. Used to match compatible people.</p>
          <div className="flex flex-wrap gap-2">
            {LOOKING_FOR_OPTIONS.map((o) => {
              const active = (profile.looking_for ?? []).includes(o.value);
              return (
                <Badge
                  key={o.value}
        ...
        <section className="bg-card rounded-2xl p-6 shadow-card space-y-4">
          <h3 className="font-heading font-semibold text-foreground flex items-center gap-2">
            <Globe className="h-5 w-5" /> Match preferences
          </h3>
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(v) => updateProfile({ gender_preference: v }, 'Preference saved')}
            >
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(v) => updateProfile({ location_preference: v }, 'Preference saved')}
            >
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <Button size="sm" variant="outline" onClick={saveLocation}>Save location</Button>

          <div className="grid grid-cols-3 gap-3">
            <div className="space-y-2">
              <Label className="text-sm">Your age</Label>
              <Input value={age} onChange={(e) => setAge(e.target.value)} inputMode="numeric" placeholder="—" />
            </div>
            <div className="space-y-2">
              <Label className="text-sm">Min age</Label>
              <Input value={ageMin} onChange={(e) => setAgeMin(e.target.value)} inputMode="numeric" placeholder="18" />
            </div>
            <div className="space-y-2">
              <Label className="text-sm">Max age</Label>
              <Input value={ageMax} onChange={(e) => setAgeMax(e.target.value)} inputMode="numeric" placeholder="99" />
            </div>
          </div>
          <Button size="sm" variant="outline" onClick={saveAges}>Save ages</Button>
        </section>
